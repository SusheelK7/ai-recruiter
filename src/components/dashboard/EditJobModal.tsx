...
import { experienceLevels, type UpdateJobInput } from "@/lib/validations/job";
...
type FormErrors = Partial<Record<keyof UpdateJobInput | "requiredSkills" | "root", string>>;
...
export function EditJobModal({ open, job, onClose, onJobUpdated }: EditJobModalProps) {
    ...
    const fetchJobDetails = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const j = data.job;
          setTitle(j.title || job.title);
          setDescription(j.description || "");
          setSkillsInput(
            Array.isArray(j.requiredSkills) ? j.requiredSkills.join(", ") : ""
          );
          if (j.experienceLevel && experienceLevels.includes(j.experienceLevel)) {
            setExperienceLevel(j.experienceLevel);
          }
          if (j.expiryDate) {
            setCustomExpiryDate(j.expiryDate.split("T")[0]);
            setExpiryDays(null);
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload = {
      ...
      requiredSkills,
      experienceLevel,
      expiryDays: customExpiryDate ? null : expiryDays,
    ...
    try {
      ...
      onJobUpdated({
        ...
        applicationsCount: updated.applicationsCount,
        daysUntilExpiry: updated.expiryDate
          ? Math.ceil((new Date(updated.expiryDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
          : null,
      ...
      setErrors({ root: "Failed to update job. Please try again." });
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4">
      ...
      <div className="relative z-10 max-h-[92vh] w-full overflow-y-auto rounded-t-2xl bg-[var(--bg-card)] p-4 shadow-2xl transition-colors duration-300 sm:max-h-[90vh] sm:max-w-2xl sm:rounded-2xl sm:p-6">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-[var(--text-primary)]">Edit Job Posting</h2>
            <p className="mt-1 text-sm text-[var(--text-muted)]">
              Update details, skills, or expiry date for this role.
            </p>
          </div>
          <button
            type="button"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                rows={12}
                placeholder="Type your keywords or short summary notes here (e.g., Remote, US $120k, 5+ yrs exp), then click 'Generate with AI'..."
                className="min-h-[220px] w-full rounded-xl border border-zinc-200 bg-white px-3.5 py-2.5 font-mono text-xs leading-relaxed text-zinc-900 transition-all outline-none focus:border-[#2E5B8A] focus:ring-2 focus:ring-[#2E5B8A]/20 sm:text-sm dark:border-zinc-700/80 dark:bg-[#1A2233] dark:text-zinc-100 dark:focus:border-[#4A7FC1] dark:focus:ring-[#4A7FC1]/25"
              />
              {errors.description && (
